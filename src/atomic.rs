...
fn atomic_compare_exchange<T: PartialEq>(ptr: *mut c_void, expected: *mut c_void, desired: T) -> bool {
    unsafe {
...
fn atomic_fetch_nand<T: BitAnd<Output = T> + Not<Output = T> + Copy>(ptr: *mut c_void, val: T) -> T {
    unsafe {
        let dst = ptr as *mut T;
        let old: T = dst.read();
        dst.write(!(old & val));
...
fn atomic_nand_fetch<T: BitAnd<Output = T> + Not<Output = T> + Copy>(ptr: *mut c_void, val: T) -> T {
    unsafe {
        let dst = ptr as *mut T;
        let old: T = dst.read();
        let val2 = !(old & val);
